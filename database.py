...
class DatabaseHandler:
    ...
    def _create_tables(self):
        ...
        try:
            ...
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.table_name}_src ON {self.table_name} (src)")
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.table_name}_dst ON {self.table_name} (dst)")
    ...
    def save_message(self, msg_id: str | None, source: str | None, dest: str | None, msg_type: str | None, received_time: datetime, raw_message_str: str):
        """
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute(f"""
            INSERT INTO {self.table_name} (msg_id, src, dst, type, received_at, raw_message)
            VALUES (?, ?, ?, ?, ?, ?)
            """, (msg_id, source, dest, msg_type, received_time, raw_message_str))
            self.connection.commit()
