...
def setup_logging(logging_config: SimpleNamespace):
    ...
    try:
        ...
        try:
            ...
            if not when:
                log.warning(
                    "Ungültiges 'rolling_interval': '%s' in der Konfiguration. Verwende 'day' (D) stattdessen.",
                    interval_setting,
                )
            ...
            log.debug(
                "File-Handler konfiguriert für '%s' mit Level %s, Rotation '%s', %d Backups.",
                log_file_path,
                file_level_str,
                when,
                backup_count,
            ...
            log.error(
                "Keine Berechtigung zum Erstellen/Schreiben der Log-Datei '%s'. Datei-Logging deaktiviert. %s",
        ...
        log.critical(
            "Fehler: Fehlende Konfigurationseinstellung im logging_config Objekt: %s",
            e,
            exc_info=True,
        )
